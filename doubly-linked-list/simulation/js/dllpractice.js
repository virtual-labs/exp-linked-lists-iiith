  ...
  imgdeclarer();
    function drawBox(x, y, ind, color) {
    	...
    	ctx.globalAlpha = 0.5;
    	ctx.drawImage(boxline, x+linegap*.95, y+(nHeight-lineheight)/2, linewidth, lineheight);
    	ctx.drawImage(boxline, x+linegap*.4, y+(nHeight-lineheight)/2, linewidth, lineheight);
    	ctx.globalAlpha = 1;
    
    	if (ind==numbers.length-1)	
    	{
    		ctx.drawImage(arrowtriangle, x+(linegap+15+43+45)*.9, y+(nHeight-dotheight)/2-10, 20, 14);
    		ctx.drawImage(arrowline, x+linegap+15, y+(nHeight-dotheight)/2-5, 100*0.7, 5);
    	}
    	else
    	{
    		ctx.drawImage(arrowtriangle, x+(linegap+15+43)*.9, y+(nHeight-dotheight)/2-10, 20, 14);
    		ctx.drawImage(arrowline, x+linegap+15, y+(nHeight-dotheight)/2-5, 55*0.6, 5);
    	}
    
    	ctx.drawImage(dot, x+(linegap+15)*.9, y+(nHeight-dotheight)/2-10, dotwidth, dotheight);
    	ctx.drawImage(dot, x+12, y+(nHeight-dotheight)/2+10, dotwidth, dotheight);
    	
    
    	ctx.drawImage(arrowline, x+13-55*0.6, y+(nHeight-dotheight)/2+15, 55*0.6, 5);
    
    	ctx.drawImage(reversetriangle, x-55*0.6+5, y+(nHeight-dotheight)/2+12, 20, 14);
    	...
    	ctx.fillText(numbers[ind], x+(linegap*.55-ll)/2+linegap*.4, (rectHeight)+y);
    	ctx.fill(); 
    	ctx.closePath();
    	ctx.stroke();
