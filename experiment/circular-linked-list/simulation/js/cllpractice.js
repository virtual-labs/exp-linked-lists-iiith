...
function drawBox(x, y, ind, color) {
  ...
  if (ind == numbers.length - 1) {
    ctx.beginPath();
    ctx.lineWidth = 1;
    ctx.strokeStyle = "#979091";
    ctx.moveTo(x + (linegap + 15) * 0.9 + 1, y + (nHeight - dotheight) / 2);
    ctx.lineTo(x + (linegap + 15) * 0.9 + 50, rectHeight / 2 + y + 10);
    ctx.lineTo(x + (linegap + 15) * 0.9 + 50, rectHeight / 2 + y + 65);
    ctx.lineTo(65, rectHeight / 2 + y + 65);
    ctx.lineTo(65, rectHeight / 2 + y + 10);
    ctx.stroke();

    ctx.closePath();
    drawArrow(65, rectHeight / 2 + y + 10, 120, rectHeight / 2 + y + 10);
  } else {
    ctx.drawImage(
      arrowtriangle,
      x + (linegap + 15 + 43) * 0.9,
...
function insertAtNode() {
  ...
  if (isNaN(idx) || isNaN(val) || idx < 0 || idx > numbers.length) {
    document.getElementById("ins").innerHTML =
      "Invalid index or value for insert at node.";
    busy = 0;
    clear();
    return;
  }
